...
4. 在Windows上布署时，注意路径需要采用Windows的路径格式，不要看人家bash给你显示的路径是"/c/Users/Administrator"这种，你自己不要按这个写路径，还是要按Windows的格式，比如`acme.sh --issue -d api.231l.net --webroot "H:/nginx/html/"`
6. http-01认证只适用于网站，不适用于websocket、http api等基于http协议但本质不是网站的服务。而且你需要对网站的根目录有控制权。如果你的网站服务器不是nginx或apache，选择直接签发。
7. 如果CA是let's encrypt，http-01认证要求网站必须在80端口运行，或者网站能够通过80重定向到对应端口，网站无法通过80端口访问是不行的
8. 务必按照教程修改默认CA
9. 复制证书时，你会发现acme.sh会生成.cer结尾的文件。如果网站程序要求提供.crt文件，直接修改.cer后缀名为.crt即可。如果网站程序要求提供.pem文件，需要用openssl软件进行转换。

## 关于Let's Encrypt的注意事项

由于大家使用最多的是Let's Encrypt，这里给出一些这家CA的相关注意事项

1. Let's Encrypt 对证书申请有一些速率限制，以确保服务的公平使用
    - 每个账户每 3 小时最多可申请 300 份证书
    - 每个注册域名每 7 天最多可申请 50 份证书
    - 每个域名集合每 7 天最多可申请 5 份证书  
此外用同一个帐号对同一个域名申请证书如果频繁失败，Let's Encrypt也会暂时禁止对应域名的证书申请。如果你是个手残党，建议合理安排时间上后台申请证书，避免因为手残申请失败次数过多被ban。
