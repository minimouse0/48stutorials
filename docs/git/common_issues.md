...
## 添加子模块时提示`fatal: 'lib' already exists in the index `

出现这个提示的原因是：Git 的暂存区（index）里已经存在名为 lib 的路径记录。

你可以按照以下步骤来解决：

1. 先将 lib 路径从 Git 的暂存区中移除：
    `git rm --cached -r 子模块文件夹名`

2. 如果之前尝试添加子模块留下过残留，清理一下子模块缓存（若提示文件不存在可忽略）：
    `rm -rf .git/modules/子模块文件夹名`
